import { TypedArray, TypedArrayConstructor, wasmSourceMap } from 'utils'
...
if (import.meta.env && import.meta.env.MODE !== 'production') {
  const hex = (await import('../as/build/seq.wasm?raw-hex')).default
  const fromHexString = (hexString: string) => Uint8Array.from(
    hexString.match(/.{1,2}/g)!.map(byte =>
      parseInt(byte, 16)
    )
  )
  const wasmMapUrl = new URL('/as/build/seq.wasm.map', location.origin).href
